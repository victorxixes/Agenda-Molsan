    ...
    try:
        while True:
            try:
                ...
                # ---------------------------------------------------------
                # 2) ENVÍO DE MENSAJE DE TEXTO (REALTIME COMPLETO)
                # ---------------------------------------------------------
                elif tipo == "mensaje":
                    ...
                    # 🔥 Enviar al remitente para actualizar su chat
                    await manager.send_to_user(remitente_id, {
                        "tipo": "nuevo_mensaje",
                        "de": remitente_id,
                        "contenido": contenido
                    })
                ...
                # ---------------------------------------------------------
                # 3) ENVÍO DE ARCHIVO (REALTIME COMPLETO)
                # ---------------------------------------------------------
                elif tipo == "archivo":
                    remitente_id = empleado_id
                    destinatario_id = data.get("destinatario_id")
