...
export default function SeguridadFicha({ empleadoId }) {
  ...
  const permisosEmpleado =
    ...
    fichaSegura.permisos_modulo_dict !== null
  ...
  const permisosGlobales = useMemo(() => {
    ...
    return permisos.reduce((acc, p) => {
      ...
      if (typeof p.modulo !== "string" || typeof p.permiso !== "string")
        return acc;
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      {/* AQUÍ SIGUE TU PARTE 2 (ya blindada antes) */}
    </div>
  );
  {/* SEGURIDAD */}
<div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl shadow-xl p-6 space-y-6">
  ...
  <div>
    ...
    <button
      ...
      onClick={() => {
        if (typeof empleado.id === "number" && nuevaPassword.trim() !== "") {
  ...
  <div>
    ...
    <button
      ...
      onClick={() => {
        const rolNum = Number(nuevoRol);
        if (!isNaN(rolNum) && typeof empleado.id === "number") {
          asignarRol(empleado.id, rolNum);
        }
        setNuevoRol("");
      }}
    >
  ...
  {showModulos && (
    <ul className="space-y-3 text-white">
      {Object.keys(permisosGlobales || {}).map((modulo) => {
        ...
        const visible = Array.isArray(modulosVisibles)
  ...
  {showPermisos && (
    <ul className="space-y-6 text-white">
      {(Object.entries(permisosGlobales || {}) || []).map(([modulo, permsDisponibles]) => {
        ...
        const listaPerms = Array.isArray(permsDisponibles)
          ? permsDisponibles
          : [];
          ...
          <li key={modulo}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
              {listaPerms.map((perm) => {
                ...
                const checked = Array.isArray(permisosEmpleado[modulo])
                  ? permisosEmpleado[modulo].includes(perm)
                  : false;
  ...
  {showLogs && (
    <>
      ...
      <table className="w-full text-sm bg-white/5 border border-white/10 rounded-xl text-white overflow-hidden">
        ...
        <tbody>
          {(logsPaginados || []).map((l) => {
            ...
            const fecha = typeof l.fecha === "string" ? l.fecha : "-";
            const evento = typeof l.evento === "string" ? l.evento : "-";
            const detalle = typeof l.detalle === "string" ? l.detalle : "-";
            const ip = typeof l.ip === "string" ? l.ip : "-";
            ...
            return (
              <tr key={key} className="border-b border-white/10 hover:bg-white/5 transition">
                ...
                <td className="p-2">
                  {(iconosEvento[evento] || iconosEvento.default)} {evento}
                </td>
      ...
      <div className="flex items-center gap-3 mt-4 text-white">
        ...
        <button
          disabled={(paginaLog + 1) * pageSizeLog >= (logsOrdenados?.length || 0)}
          onClick={() => setPaginaLog(paginaLog + 1)}
          className="px-3 py-1 bg-white/10 border border-white/20 rounded-xl disabled:opacity-40 hover:bg-white/20 transition active:scale-[0.97]"
        >
          Siguiente →
