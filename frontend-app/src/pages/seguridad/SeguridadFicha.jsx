    ...
    <div className="p-6 space-y-6">
      ...
      >
        ...
        {showPermisos && (
          <ul className="space-y-6 text-white">
            {Object.entries(permisosGlobales).map(
              ([modulo, permsDisponibles]) => (
                <li key={modulo}>
      ...
      >
        ...
        <div className="flex items-center gap-3 mt-4 text-white">
          ...
          <button
            disabled={
              (paginaAud + 1) * pageSizeAud >= auditoriaOrdenada.length
            }
            onClick={() => setPaginaAud(paginaAud + 1)}
            className="
              px-3 py-1 bg-white/10 border border-white/20 rounded-xl
      ...
      >
        ...
        <div className="flex items-center gap-3 mt-4 text-white">
          ...
          <button
            disabled={
              (paginaLog + 1) * pageSizeLog >= logsOrdenados.length
            }
            onClick={() => setPaginaLog(paginaLog + 1)}
            className="
              px-3 py-1 bg-white/10 border border-white/20 rounded-xl
